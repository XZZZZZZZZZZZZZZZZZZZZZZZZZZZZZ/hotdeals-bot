...
async function generateMarketingText(title, price) {
    ...
    try {
        const prompt = `כתוב פוסט שיווקי לערוץ דילים.
מוצר: ${title}
מחיר: ₪${price}
דרישות: עברית רהוטה, שימוש באימוג'ים, רשימת יתרונות וסיכום אטרקטיבי.`;
    ...
    try {
        ...
        if (whatsappReady && targetGroupId) {
            try {
                ...
                console.log("שגיאת שליחה בוואטסאפ:", wErr.message);
            }
        }
...
whatsapp.on('ready', async () => {
    ...
    try {
        ...
        if (group) {
            ...
            console.log(`🎯 קבוצה נמצאה: ${targetGroupId}`);
            whatsappReady = true;
            fetchDeal();
        } else {
            console.log("❌ לא מצאתי את הקבוצה לפי השם.");
        }
    } catch (e) {
        console.log("שגיאה באיתור קבוצה:", e.message);
    }
});
