...
function loadKeywords() {
  try {
    ...
    // התיקון הקריטי: מוודא שהקוד לא יחפש .keywords אם הקובץ הוא כבר מערך
...
function getNextKeyword() {
  ...
  let selected;
  // הגנה מפני לולאה אינסופית אם יש רק מילה אחת
...
async function fetchDeal() {
  ...
  try {
    ...
    for (const product of products) {
      if (!product.product_id || !product.product_detail_url || !product.product_main_image_url) continue;
